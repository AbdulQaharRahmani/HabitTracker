...
export default function AddCategory({ variant = "card" }) {
    ...
    <>
      {variant === "header" && (
        <button
          ...
          className="bg-indigo-500 hover:bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold flex items-center gap-2 shadow-md transition-transform active:scale-95"
        >
          <LuPlus size={18} />
          {t("Add Category")}
        </button>
            ...
            <div className="p-4">
              ...
              <div className="my-2">
                <div className="flex flex-row items-center bg-gray-50 dark:bg-gray-900/50 justify-start text-md text-gray-500 dark:text-gray-400 rounded-lg py-2 border border-dashed border-gray-200 mb-2">
                  <p className="mx-3 text-xs font-bold text-gray-500 dark:text-gray-400">
                    {t("Category Name")}:
                  </p>
                  <input
                    value={categoryName}
                    onChange={(e) => setCategoryName(e.target.value)}
                    className={`p-2 ${isRTL ? "pl-6" : "pr-6"} outline-none bg-gray-50 dark:bg-gray-900/50 dark:text-white`}
                    placeholder={t("Enter category name...")}
                    onKeyDown={(e) => e.stopPropagation()}
                  />
                </div>
                <div className="text-sm text-gray-500 dark:text-gray-400 border border-dashed border-gray-200 rounded-lg bg-gray-50 py-2">
                  <p className="mx-3 text-xs font-bold text-gray-500 dark:text-gray-400 my-4">
                    {t("Pick icon")}:
                  </p>
